...
export function withRouter (app, { name, ...config }) {
  return function (state, actions, view, root) {
    const router = routerFactory({ ...config, pathFn: (state) => state[name] })
    state[name] = router.state
    actions[name] = router.actions
    return router.subscribe(app(state, actions, view, root))
  }
}
